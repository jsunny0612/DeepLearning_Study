...
from data.MNIST.mnist_data_aug import CustomMNIST_Aug
from data.cifar10.cifar10_data_aug import CustomCIFAR10_Aug
...
def set_dataloader(dataset,data_path):
    if dataset == 'mnist':
        train_dataset = CustomMNIST_Aug(root=data_path, train=True)
        test_dataset = CustomMNIST_Aug(root=data_path, train=False)

    elif dataset == 'cifar10':
        train_dataset = CustomCIFAR10_Aug(root=data_path, train=True)
        test_dataset = CustomCIFAR10_Aug(root=data_path, train=False)
...
def main():
    ...
    for epoch in range(epochs):
        ...
        loss_name = f'Total_{args.dataset}_{args.model}_Loss'
        accuracy_name = f'Total_{args.dataset}_{args.model}_Accuracy'
        ...
        # model.load_state_dict(torch.load(f'./model_weights_epoch_{epoch + 1}.pth'))
