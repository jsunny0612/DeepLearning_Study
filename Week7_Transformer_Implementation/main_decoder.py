...
from model.decode_method import greedy_search, beam_search
...
def parse_args():
    ...
    parser.add_argument("--decode_method", type=str, choices=["greedy", "beam"], default="greedy", help="choose decode method")
...
def set_dataloader(batch_size=32, max_len=300, model_name="t5-small"):
...
def decode(outputs, tokenizer, method="greedy", eos_token_id=None):
    if method == "greedy":
        return greedy_search(outputs, tokenizer)
    elif method == "beam":
        # return beam_search(outputs, k=2 , eos_token_id=eos_token_id)
        return beam_search(outputs, k=2)


def train(model, train_loader, criterion, optimizer, tokenizer, device, decode_method):
    model.train()
    train_loss = 0
    ...
    for batch in tqdm(train_loader):  # Log progress
        ...
        generated_tokens = decode(outputs, tokenizer, method=decode_method, eos_token_id=tokenizer.eos_token_id)
        for i in range(target.size(0)):
            ...
            train_total_bleu4 += bleu4
            num_bleu_samples += 1


    train_loss /= len(train_loader)
    avg_bleu_score4 = (train_total_bleu4 / num_bleu_samples)
    print(f'Train Loss: {train_loss:.4f}, Train BLEU-4 Score: {avg_bleu_score4:.2f}%')
    return train_loss, avg_bleu_score4


def validate(model, val_loader, criterion, tokenizer, device, decode_method):
    model.eval()
    val_loss = 0
    smoothing_function = SmoothingFunction().method1
    valid_total_bleu4 = 0
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader):  # Log progress
            source = batch["source"].to(device)
            target = batch["target"].to(device)
            ...
            for i in range(target.size(0)):
                ...
                bleu4 = sentence_bleu([ref], hyp, weights=(0.25, 0.25, 0.25, 0.25), smoothing_function=smoothing_function)
                valid_total_bleu4 += bleu4
                num_bleu_samples += 1


    val_loss /= len(val_loader)
    avg_bleu_score4 = (valid_total_bleu4 / num_bleu_samples)
    print(f'Validation Loss: {val_loss:.4f}, Validation BLEU-4 Score: {avg_bleu_score4:.2f}%')
    return val_loss, avg_bleu_score4


def test(model, test_loader, criterion, tokenizer, device, decode_method,data_results):
    model.eval()
    test_loss = 0
    smoothing_function = SmoothingFunction().method1
    test_total_bleu4 = 0
...
def main():
    ...
    print("Dataset: WMT16Dataset ")
    print(f"Decoding method selected: {args.decode_method}")


    train_loader, val_loader, test_loader, tokenizer = set_dataloader(
        batch_size=32, max_len=300, model_name="t5-small")
    vocab_size = tokenizer.vocab_size

    model = set_model(n_input_vocab=vocab_size, n_output_vocab=vocab_size,
                      d_model=512, head=8, d_ff=2048, max_len=300,
    ...
    for epoch in range(epochs):
        ...
        train_loss, train_bleu = train(model, train_loader, criterion, optimizer, tokenizer, device, args.decode_method)
        val_loss, val_bleu = validate(model, val_loader, criterion, tokenizer, device, args.decode_method)

        writer.add_scalars(f"Transformer_{args.decode_method}/Loss", {'Train': train_loss,'Validation': val_loss}, epoch + 1)
        writer.add_scalars(f"Transformer_{args.decode_method}/BLEU", {'Train': train_bleu,'Validation': val_bleu}, epoch + 1)

        scheduler.step()

    print("\n======= Fianl Test Evaluation =======")
    test_loss, test_bleu = test(model, test_loader, criterion, tokenizer, device, args.decode_method, data_results)
    ...
    results_filename = f"test_results_{args.decode_method}.json"
