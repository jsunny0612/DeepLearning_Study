''' Define the P-Transformer model '''

import torch
import torch.nn as nn
from Week9_P_Transformer.model.P_Layer import EncoderLayer,DecoderLayer
...
class P_Transformer(nn.Module):
    def __init__(self, n_input_vocab, n_output_vocab, d_model, head, d_ff, max_len,
                 padding_idx, dropout, n_layers, n_parallel_encoders, n_parallel_decoders, device):
        super().__init__()
        ...
        self.encoders = nn.ModuleList([
            Encoder(n_input_vocab=n_input_vocab,
                    d_model=d_model,
                    head=head,
                    d_ff=d_ff,
                    max_len=max_len,
                    ...
                    n_layers=n_layers,
                    device=device)
            for _ in range(n_parallel_encoders)
        ])

        # Decoder
        self.decoders = nn.ModuleList([
            Decoder(n_output_vocab=n_output_vocab,
                    d_model=d_model,
                    head=head,
                    d_ff=d_ff,
                    max_len=max_len,
                    ...
                    n_layers=n_layers,
                    device=device)
            for _ in range(n_parallel_decoders)
        ])
    ...
    def forward(self, source, target):
        ...
        for encoder, decoder in zip(self.encoders, self.decoders):
            encoder_output = encoder(source, padding_mask)
            decoder_output = decoder(target, encoder_output, look_ahead_mask, enc_dec_padding_mask)
            output_decoders.append(decoder_output)

        final_output = output_decoders[-1]
        output = self.linear(final_output)  # (batch_size, seq_len, vocab_size)
        return output
    ...
    def forward(self, source, target):
        ...
        output_encoders = [encoder(source, padding_mask) for encoder in self.encoders]
        output_decoders = []

        for i, decoder in enumerate(self.decoders):
            decoder_output = decoder(target, output_encoders[i], look_ahead_mask, enc_dec_padding_mask)
            output_decoders.append(decoder_output)
